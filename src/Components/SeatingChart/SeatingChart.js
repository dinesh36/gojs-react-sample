...
function SeatingChart({guestList}){
  const $ = go.GraphObject.make;
  const initTableDiagram = () =>{
  ...
  const initGuestDiagram = ()=>{
    // initialize the Palette
    myGuests =
      $(go.Diagram,
        {
          layout: $(go.GridLayout, {sorting: go.GridLayout.Ascending}),
  ...
  function initChartData() {
    ...
    myDiagram.model = new go.GraphLinksModel([
      {"key": 3, "category": "TableR8", "name": "Table", "guests": {}, "loc": "121.5 203.5"},
    ]);  // this sample does not make use of any links

    myGuests.nodeTemplateMap = myDiagram.nodeTemplateMap;

    // specify the contents of the Palette
    myGuests.model = new go.GraphLinksModel(guestList);
...
// This returns null if no seat is available at this table.
  ...
  return <div id="sample">
    <div id="myFlexDiv">
      <ReactDiagram initDiagram={initGuestDiagram} divClassName='guestsDiv'/>
      <ReactDiagram initDiagram={initTableDiagram} divClassName='tableDiagramDiv'/>
    </div>
    <div>
      <pre id="savedModel" style={{height: '250px'}} />
    </div>
  </div>;
}
